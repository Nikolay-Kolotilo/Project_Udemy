...
Приведу наглядный пример из этой статьи. (Пока её можно открыть только в начале, 
...
То есть, организовывать композицию функций. Обратите внимание на порядок записи 
функций - последняя записанная запускается первой и дальше справа налево. Возможно вам понадобится это.
*/
...
А теперь напишите функцию композиции composeWithArgs, которая принимает сколько угодно аргументов в начале.
Пример:*/
...
const composeWithArgs = function (x) {
    return function (...fns) {
        return fns.reduce((res, fn) => res + fn, 1);
    };
...
const compose = (...fns) => (x) => fns.reduceRight((res, fn) => fn(res), x);// Задача №1

const composeWithArgs = (...fns) => fns.reduceRight((f, g) => (...args) => g(f(...args)));// Задача №2